...
.hdr-r{width:28px;height:28px;border:none;border-radius:6px;background:transparent;color:#94a3b8;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center}
.hdr-r:hover{background:#f1f5f9;color:#475569}
...
.info.act{border-color:#6366f1;color:#1a1a2e}
...
<div class="hdr"><div class="hdr-l">🤖 AI Agent</div><button class="hdr-r" id="cl">✕</button></div>
<div class="body">
  <div class="status-bar"><span class="dot idle" id="dot"></span><span class="status" id="st">等待连接项目</span></div>
  <div class="row">
    <button class="btn btn-p" id="sel">📁 选择项目</button>
    <div class="info" id="info"><span class="lbl">未选择</span></div>
  </div>

  <div class="pblock hidden" id="pb">
    <div class="phdr">📋 发送提示词给 AI</div>
    <div class="ptext" id="pt"></div>
    <div class="pacts">
      <button class="btn btn-g" id="ci">📝 填入输入框</button>
      <button class="btn btn-o" id="cc">📋 复制</button>
    </div>
  </div>

  <div class="log">
    <div class="log-h"><span class="log-t">操作日志</span><button class="btn btn-o" id="clr" style="font-size:11px;padding:3px 8px">清除</button></div>
    <div class="log-b" id="lb"><div class="empty">连接项目后这里显示操作记录</div></div>
  </div>
</div></div></div>`
  ...
  private bindEvents():void{
    ...
    this.elSelect.addEventListener('click',async()=>{if(!this.onSelectProject)return;this.elSelect.disabled=true;this.elSelect.textContent='⏳...';try{const r=await this.onSelectProject();if(r)this.setProjectInfo(r.name,r.fileCount)}catch(e){this.addLog('error',`❌ ${(e as Error).message}`)}finally{this.elSelect.disabled=false;this.elSelect.textContent='📁 选择项目'}})
    this.byId('clr').addEventListener('click',()=>this.clearLogs())
    this.byId('ci').addEventListener('click',()=>{if(!this.elPromptText.textContent)return;this.onCopyToInput?.(this.elPromptText.textContent);this.addLog('success','📝 已填入输入框')})
    this.byId('cc').addEventListener('click',()=>{const t=this.elPromptText.textContent;if(!t)return;navigator.clipboard.writeText(t).then(()=>this.addLog('success','📋 已复制到剪贴板'))})
  }
  ...
  setProjectInfo(name:string,count:number):void{this.elInfo.innerHTML=`<span class="lbl">项目:</span> ${esc(name)}  │  <span class="lbl">文件:</span> ${count}`;this.elInfo.classList.add('act')}
  ...
  clearLogs():void{this.elLog.innerHTML='<div class="empty">已清除</div>'}
