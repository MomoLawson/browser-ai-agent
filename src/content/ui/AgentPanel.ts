...
const T:Record<UILang,Record<string,string>>={
  ...
  'zh-CN':{wa:'等待连接项目',se:'选择项目',ns:'未选择',sp:'发送提示词给 AI',fi:'填入输入框',cp:'复制',lg:'操作日志',cl:'清除',cd:'已清除',ch:'连接项目后这里显示操作记录',st:'设置',ll:'界面语言',cs:'关闭',fd:'已填入',co:'已复制'},
  'zh-TW':{wa:'等待連接專案',se:'選擇專案',ns:'未選擇',sp:'發送提示詞給 AI',fi:'填入輸入框',cp:'複製',lg:'操作日誌',cl:'清除',cd:'已清除',ch:'連接專案後這裡顯示操作記錄',st:'設定',ll:'介面語言',cs:'關閉',fd:'已填入',co:'已複製'},
}
...
<div class="sp hidden" id="sp"><h3><i data-lucide="sliders"></i>${this.t('st')}</h3><label>${this.t('ll')}</label><select id="langSel"></select><button class="btn btn-p sp-close" id="spCl"><i data-lucide="check"></i>${this.t('cs')}</button></div>`
  ...
  setStatus(s:'idle'|'listening'|'error',t?:string):void{this.elDot.className=`dot ${s}`;if(t)this.elST.textContent=t}
